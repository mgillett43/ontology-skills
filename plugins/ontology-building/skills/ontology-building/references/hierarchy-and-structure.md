...
- **Multiple inheritance is legitimate when genuinely true, but it is far more often a symptom** — a concept that really is two things at once should have both parents, not one arbitrarily chosen. But most multiple inheritance in practice is an unresolved decision in disguise, so run test 5 below before accepting it. Sharpen the genuine cases with UFO's **Role vs. RoleMixin** distinction: a RoleMixin is a role that can be played by instances of otherwise-disjoint parent types (e.g., "Customer" playable by both a Person and an Organization) — model it as a role cutting across the type hierarchy, not as an awkward shared parent or a duplicated role per playing type.
...
4. **The multiple-parent test.** Whenever a class ends up with more than one parent, treat it as suspect until it passes all five of these — spurious cases usually fail at least one, and each edge looks locally plausible on its own, so the pathology is only visible when you check them together:
   - **Litmus each parent separately.** "Every [Child] is a [Parent]" must hold, permanently and necessarily, for *each* parent independently.
   - **Rigidity.** Is any parent actually a role or a phase? If so it isn't a parent — extract a role assignment instead. This is the single commonest cause.
   - **Contribution.** Does each parent contribute properties or constraints the child genuinely has? A parent that contributes nothing is a label, not a type.
   - **Latent disjointness.** Would the parents be disjoint if you asserted disjointness? If so the class is already unsatisfiable; nothing is stopping you noticing yet.
   - **Provenance.** Where did each parent come from? If each traces to a different source — a different document, system, or team — and no reconciliation decision exists, this is unresolved synonymy, not multiple inheritance. Resolve to one term and map the others (`reuse-and-alignment.md`).

   At model scale, check the *distribution* rather than individual cases: a population of classes with three or more parents is a structural signal, not a set of independent judgement calls.
5. **The three-way placement test for an external checklist or methodology.** Incorporating an external framework wholesale would otherwise mean minting an ontology class for the whole methodology. Split each item instead into: purely procedural (stays a guided process/skill outside the ontology), formally checkable (becomes a graph/schema constraint), or evidential (an activity producing ontology-native outputs, such as issues or hypotheses). This avoids modeling a methodology as if it were a domain entity.
