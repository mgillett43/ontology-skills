...
license: CC-BY-4.0. LICENSE has complete terms. Attribution required — see README
...
| [categories-and-relations.md](references/categories-and-relations.md) | Seven plain-language checks per entity — is it a thing or a happening, what inheres in it, where and when it is, what it's about |
...
4. **Define classes, hierarchy, and relations — and write their definitions.** Apply the is-a litmus test, the sibling-generality check, and the subclass-vs-property test to every candidate; write a genus-differentia definition for each construct as you mint it, not afterwards. Run the per-entity checks in `references/categories-and-relations.md` inline — ask only the ones that would change what you build. **Hooks (tags only — no schema commitment yet):** flag anything that *changes state in the world* (rather than merely recording a fact) as an **action-candidate**; flag any property whose value could legitimately differ by source, be disputed, or need tracing back to who asserted it as a **statement-candidate**. → `references/hierarchy-and-structure.md`, `references/definitions-and-documentation.md`
